...
preferences = App.ParamGet("User parameter:BaseApp/Preferences/Mod/FreeCAD-CommandTab")
...
class Settings:
    ...
    def GetBoolSetting(settingName) -> bool | None:
        result = None
    ...
    def SetBoolSetting(settingName, value: bool | None):
        if value is None:
            value = False
        preferences.SetBool(settingName, bool(value))
        App.saveParameter()
        return
    ...
    def WriteSettings():
        ...
        Settings.SetStringSetting("CommandTabStructure", COMMANDTAB_STRUCTURE_JSON)
        ...
        Settings.SetBoolSetting("AutoHideCommandTab", AUTOHIDE_COMMANDTAB)
        ...
        Settings.SetIntSetting("CommandTab_Scroll", COMMANDTAB_SCROLLSPEED)
        Settings.SetIntSetting("TabBar_Click", TABBAR_CLICKSPEED)
        Settings.SetIntSetting("CommandTab_Click", COMMANDTAB_CLICKSPEED)
        Settings.SetStringSetting("Shortcut_Application", SHORTCUT_APPLICATION)
        Settings.SetBoolSetting(
            "ShowFavoritesInApplicationMenu", SHOW_FAVORITES_IN_APP_MENU
        )
        Settings.SetBoolSetting(
            "ShowQuickAccessInApplicationMenu", SHOW_QUICKACCESS_IN_APP_MENU
        )
        Settings.SetBoolSetting(
            "ShowCurrentWorkbenchInApplicationMenu",
            SHOW_CURRENT_WB_IN_APP_MENU,
        )
        Settings.SetBoolSetting(
            "ShowRecentWorkbenchesInApplicationMenu",
            SHOW_RECENT_WB_IN_APP_MENU,
        )
        Settings.SetBoolSetting(
            "ShowWorkbenchSwitcherInApplicationMenu",
            SHOW_ALL_WB_IN_APP_MENU,
        )
        Settings.SetBoolSetting(
            "ModernApplicationMenuEnabled", MODERN_APPLICATION_MENU_ENABLED
        )
        Settings.SetBoolSetting("ModernDialogsEnabled", MODERN_DIALOGS_ENABLED)
        Settings.SetBoolSetting(
        ...
        Settings.SetStringSetting("NativeThemeMode", NATIVE_THEME_MODE)
        Settings.SetStringSetting("IconThemeMode", ICON_THEME_MODE)
        Settings.SetBoolSetting("PreferNativeCommandTab", PREFER_NATIVE_COMMANDTAB)
        Settings.SetBoolSetting("NativeCommandTabWarmup", NATIVE_COMMANDTAB_WARMUP)
        Settings.SetBoolSetting("TabClickPopupMode", TAB_CLICK_POPUP_MODE)
        Settings.SetBoolSetting(
            "CheckForUpdatesOnStartup", CHECK_FOR_UPDATES_ON_STARTUP
        )
        Settings.SetBoolSetting(
            "EagerLoadCommandTabDependencies", EAGER_LOAD_COMMANDTAB_DEPENDENCIES
        )
        Settings.SetBoolSetting(
            "UseExternalIconThemeFallback", USE_EXTERNAL_ICON_THEME_FALLBACK
        )
        Settings.SetIntSetting("RecentWorkbenchLimit", APP_MENU_RECENT_LIMIT)
        Settings.SetStringSetting("FavoriteCommands", FAVORITE_COMMANDS)
        Settings.SetStringSetting("PinnedWorkbenches", PINNED_WORKBENCHES)
        Settings.SetStringSetting("RecentWorkbenches", RECENT_WORKBENCHES)
...
DefaultSettings = {
    ...
    "CommandTabStructure": paths.user_state_path("CommandTabStructure.json"),
    "TabBar_Style": 0,
    "IconSize_Small": 24,
    "IconSize_Medium": 36,
    "IconSize_Large": 72,
    "ApplicationButtonSize": 40,
    "QuickAccessButtonSize": 24,
    "TabBarSize": 24,
    "RightToolbarButtonSize": 24,
    "BackupEnabled": True,
    ...
    "AutoHideCommandTab": False,
    "Stylesheet": os.path.join(os.path.join(STYLESHEET_LOCATION, "default.qss")),
    "ShowIconText_Small": True,
    "ShowIconText_Medium": True,
    "ShowIconText_Large": True,
    "MaxColumnsPerPanel": 6,
    "DebugMode": False,
    "ShowOnHover": False,
    "TabBar_Scroll": 1,
    "CommandTab_Scroll": 1,
    ...
    "ShowFavoritesInApplicationMenu": True,
    "ShowQuickAccessInApplicationMenu": True,
    "ShowCurrentWorkbenchInApplicationMenu": True,
    "ShowRecentWorkbenchesInApplicationMenu": True,
    "ShowWorkbenchSwitcherInApplicationMenu": True,
    ...
    "UseExternalIconThemeFallback": False,
    "RecentWorkbenchLimit": 6,
    "FavoriteCommands": "",
    "PinnedWorkbenches": "",
    "RecentWorkbenches": "",
...
if IMPORT_LOCATION == "" or os.path.isdir(IMPORT_LOCATION) is False:
...
if EXPORT_LOCATION == "":
    # Backward compatibility with older builds that wrote a wrong key casing.
    legacy_export_location = Settings.GetStringSetting("exportLocation")
    if legacy_export_location != "":
        EXPORT_LOCATION = legacy_export_location
        Settings.SetStringSetting("ExportLocation", EXPORT_LOCATION)
if EXPORT_LOCATION == "" or os.path.isdir(EXPORT_LOCATION) is False:
    EXPORT_LOCATION = str(DefaultSettings["ExportLocation"])
    Settings.SetStringSetting("ExportLocation", EXPORT_LOCATION)
    Settings.SetStringSetting("AddonDir", os.path.dirname(__file__))
# endregion ------------------------------------------------------------------------------------------------------------

# region - Define the CommandTab structure location ------------------------------------------------------------------------
# The location for the commandtab structure is retrieved from parameters as standard.
COMMANDTAB_STRUCTURE_JSON = Settings.GetStringSetting("CommandTabStructure")
if COMMANDTAB_STRUCTURE_JSON == "":
    COMMANDTAB_STRUCTURE_JSON = str(DefaultSettings["CommandTabStructure"])
    Settings.SetStringSetting("CommandTabStructure", COMMANDTAB_STRUCTURE_JSON)
# endregion ------------------------------------------------------------------------------------------------------------
...
if Settings.GetStringSetting("BackupFolder") == "" or os.path.isdir(BACKUP_LOCATION) is False:
    BACKUP_LOCATION = str(DefaultSettings["BackupFolder"])
    Settings.SetStringSetting("BackupFolder", BACKUP_LOCATION)
# endregion ------------------------------------------------------------------------------------------------------------

# region - CommandTab settings ---------------------------------------------------------------------------------------------
...
AUTOHIDE_COMMANDTAB: bool = Settings.GetBoolSetting("AutoHideCommandTab")
if Settings.GetBoolSetting("AutoHideCommandTab") is None:
    AUTOHIDE_COMMANDTAB = False
...
SHOW_ICON_TEXT_DEFAULTS_MIGRATED = Settings.GetBoolSetting(
    "ShowIconTextDefaultsMigrated"
)
if SHOW_ICON_TEXT_DEFAULTS_MIGRATED is None:
    # Migrate old defaults (small/medium off, large on) to the new
    # dense-but-labeled default that matches the current commandtab behavior.
    if (
        SHOW_ICON_TEXT_SMALL is False
        and SHOW_ICON_TEXT_MEDIUM is False
        and SHOW_ICON_TEXT_LARGE is True
    ):
        SHOW_ICON_TEXT_SMALL = True
        SHOW_ICON_TEXT_MEDIUM = True
        SHOW_ICON_TEXT_LARGE = True
        Settings.SetBoolSetting("ShowIconText_Small", SHOW_ICON_TEXT_SMALL)
        Settings.SetBoolSetting("ShowIconText_Medium", SHOW_ICON_TEXT_MEDIUM)
        Settings.SetBoolSetting("ShowIconText_Large", SHOW_ICON_TEXT_LARGE)
    SHOW_ICON_TEXT_DEFAULTS_MIGRATED = True
    Settings.SetBoolSetting(
        "ShowIconTextDefaultsMigrated", SHOW_ICON_TEXT_DEFAULTS_MIGRATED
    )
...
COMMANDTAB_SCROLLSPEED = Settings.GetIntSetting("CommandTab_Scroll")
if (
    Settings.GetIntSetting("CommandTab_Scroll") is None
    or Settings.GetIntSetting("CommandTab_Scroll") == 0
):
    COMMANDTAB_SCROLLSPEED = int(DefaultSettings["CommandTab_Scroll"])
    Settings.SetIntSetting("CommandTab_Scroll", COMMANDTAB_SCROLLSPEED)
...
COMMANDTAB_CLICKSPEED = Settings.GetIntSetting("CommandTab_Click")
if (
    Settings.GetIntSetting("CommandTab_Click") is None
    or Settings.GetIntSetting("CommandTab_Click") == 0
):
    COMMANDTAB_CLICKSPEED = int(DefaultSettings["CommandTab_Click"])
    Settings.SetIntSetting("CommandTab_Click", COMMANDTAB_CLICKSPEED)
...
SHOW_FAVORITES_IN_APP_MENU = Settings.GetBoolSetting(
    "ShowFavoritesInApplicationMenu"
)
if Settings.GetBoolSetting("ShowFavoritesInApplicationMenu") is None:
    SHOW_FAVORITES_IN_APP_MENU = bool(
        DefaultSettings["ShowFavoritesInApplicationMenu"]
    )
    Settings.SetBoolSetting(
        "ShowFavoritesInApplicationMenu", SHOW_FAVORITES_IN_APP_MENU
    )

SHOW_QUICKACCESS_IN_APP_MENU = Settings.GetBoolSetting(
    "ShowQuickAccessInApplicationMenu"
)
if Settings.GetBoolSetting("ShowQuickAccessInApplicationMenu") is None:
    SHOW_QUICKACCESS_IN_APP_MENU = bool(
        DefaultSettings["ShowQuickAccessInApplicationMenu"]
    )
    Settings.SetBoolSetting(
        "ShowQuickAccessInApplicationMenu", SHOW_QUICKACCESS_IN_APP_MENU
    )

SHOW_CURRENT_WB_IN_APP_MENU = Settings.GetBoolSetting(
    "ShowCurrentWorkbenchInApplicationMenu"
)
if Settings.GetBoolSetting("ShowCurrentWorkbenchInApplicationMenu") is None:
    SHOW_CURRENT_WB_IN_APP_MENU = bool(
        DefaultSettings["ShowCurrentWorkbenchInApplicationMenu"]
    )
    Settings.SetBoolSetting(
        "ShowCurrentWorkbenchInApplicationMenu", SHOW_CURRENT_WB_IN_APP_MENU
    )

SHOW_RECENT_WB_IN_APP_MENU = Settings.GetBoolSetting(
    "ShowRecentWorkbenchesInApplicationMenu"
)
if Settings.GetBoolSetting("ShowRecentWorkbenchesInApplicationMenu") is None:
    SHOW_RECENT_WB_IN_APP_MENU = bool(
        DefaultSettings["ShowRecentWorkbenchesInApplicationMenu"]
    )
    Settings.SetBoolSetting(
        "ShowRecentWorkbenchesInApplicationMenu", SHOW_RECENT_WB_IN_APP_MENU
    )

SHOW_ALL_WB_IN_APP_MENU = Settings.GetBoolSetting(
    "ShowWorkbenchSwitcherInApplicationMenu"
)
if Settings.GetBoolSetting("ShowWorkbenchSwitcherInApplicationMenu") is None:
    SHOW_ALL_WB_IN_APP_MENU = bool(
        DefaultSettings["ShowWorkbenchSwitcherInApplicationMenu"]
    )
    Settings.SetBoolSetting(
        "ShowWorkbenchSwitcherInApplicationMenu", SHOW_ALL_WB_IN_APP_MENU
    )

MODERN_APPLICATION_MENU_ENABLED = Settings.GetBoolSetting(
    "ModernApplicationMenuEnabled"
)
if Settings.GetBoolSetting("ModernApplicationMenuEnabled") is None:
    MODERN_APPLICATION_MENU_ENABLED = bool(
        DefaultSettings["ModernApplicationMenuEnabled"]
    )
    Settings.SetBoolSetting(
        "ModernApplicationMenuEnabled", MODERN_APPLICATION_MENU_ENABLED
    )

MODERN_DIALOGS_ENABLED = Settings.GetBoolSetting("ModernDialogsEnabled")
if Settings.GetBoolSetting("ModernDialogsEnabled") is None:
    MODERN_DIALOGS_ENABLED = bool(DefaultSettings["ModernDialogsEnabled"])
    Settings.SetBoolSetting("ModernDialogsEnabled", MODERN_DIALOGS_ENABLED)

MODERN_COMMANDTAB_STYLE_ENABLED = Settings.GetBoolSetting("ModernCommandTabStyleEnabled")
...
if Settings.GetBoolSetting("HideMenuBarInNativeMode") is None:
    HIDE_MENUBAR_IN_NATIVE_MODE = bool(DefaultSettings["HideMenuBarInNativeMode"])
    Settings.SetBoolSetting("HideMenuBarInNativeMode", HIDE_MENUBAR_IN_NATIVE_MODE)

NATIVE_THEME_MODE = Settings.GetStringSetting("NativeThemeMode").strip().lower()
if NATIVE_THEME_MODE not in ["auto", "dark", "light"]:
    NATIVE_THEME_MODE = str(DefaultSettings["NativeThemeMode"])
    Settings.SetStringSetting("NativeThemeMode", NATIVE_THEME_MODE)

ICON_THEME_MODE = Settings.GetStringSetting("IconThemeMode").strip().lower()
if ICON_THEME_MODE not in ["original"]:
    ICON_THEME_MODE = str(DefaultSettings["IconThemeMode"])
    Settings.SetStringSetting("IconThemeMode", ICON_THEME_MODE)

PREFER_NATIVE_COMMANDTAB = Settings.GetBoolSetting("PreferNativeCommandTab")
if Settings.GetBoolSetting("PreferNativeCommandTab") is None:
    PREFER_NATIVE_COMMANDTAB = bool(DefaultSettings["PreferNativeCommandTab"])
    Settings.SetBoolSetting("PreferNativeCommandTab", PREFER_NATIVE_COMMANDTAB)

NATIVE_COMMANDTAB_WARMUP = Settings.GetBoolSetting("NativeCommandTabWarmup")
if Settings.GetBoolSetting("NativeCommandTabWarmup") is None:
...
TAB_CLICK_POPUP_MODE = Settings.GetBoolSetting("TabClickPopupMode")
if Settings.GetBoolSetting("TabClickPopupMode") is None:
    TAB_CLICK_POPUP_MODE = bool(DefaultSettings["TabClickPopupMode"])
    Settings.SetBoolSetting("TabClickPopupMode", TAB_CLICK_POPUP_MODE)

CHECK_FOR_UPDATES_ON_STARTUP = Settings.GetBoolSetting("CheckForUpdatesOnStartup")
if Settings.GetBoolSetting("CheckForUpdatesOnStartup") is None:
    CHECK_FOR_UPDATES_ON_STARTUP = bool(DefaultSettings["CheckForUpdatesOnStartup"])
    Settings.SetBoolSetting(
        "CheckForUpdatesOnStartup", CHECK_FOR_UPDATES_ON_STARTUP
    )

EAGER_LOAD_COMMANDTAB_DEPENDENCIES = Settings.GetBoolSetting(
    "EagerLoadCommandTabDependencies"
)
if Settings.GetBoolSetting("EagerLoadCommandTabDependencies") is None:
    EAGER_LOAD_COMMANDTAB_DEPENDENCIES = bool(
        DefaultSettings["EagerLoadCommandTabDependencies"]
    )
    Settings.SetBoolSetting(
        "EagerLoadCommandTabDependencies", EAGER_LOAD_COMMANDTAB_DEPENDENCIES
    )

USE_EXTERNAL_ICON_THEME_FALLBACK = Settings.GetBoolSetting(
    "UseExternalIconThemeFallback"
)
if Settings.GetBoolSetting("UseExternalIconThemeFallback") is None:
    USE_EXTERNAL_ICON_THEME_FALLBACK = bool(
        DefaultSettings["UseExternalIconThemeFallback"]
    )
    Settings.SetBoolSetting(
        "UseExternalIconThemeFallback", USE_EXTERNAL_ICON_THEME_FALLBACK
    )

APP_MENU_RECENT_LIMIT = Settings.GetIntSetting("RecentWorkbenchLimit")
if (
    Settings.GetIntSetting("RecentWorkbenchLimit") is None
    or Settings.GetIntSetting("RecentWorkbenchLimit") <= 0
):
    APP_MENU_RECENT_LIMIT = int(DefaultSettings["RecentWorkbenchLimit"])
    Settings.SetIntSetting("RecentWorkbenchLimit", APP_MENU_RECENT_LIMIT)

FAVORITE_COMMANDS = Settings.GetStringSetting("FavoriteCommands")
if FAVORITE_COMMANDS == "":
    FAVORITE_COMMANDS = str(DefaultSettings["FavoriteCommands"])
    Settings.SetStringSetting("FavoriteCommands", FAVORITE_COMMANDS)

PINNED_WORKBENCHES = Settings.GetStringSetting("PinnedWorkbenches")
if PINNED_WORKBENCHES == "":
    PINNED_WORKBENCHES = str(DefaultSettings["PinnedWorkbenches"])
    Settings.SetStringSetting("PinnedWorkbenches", PINNED_WORKBENCHES)

RECENT_WORKBENCHES = Settings.GetStringSetting("RecentWorkbenches")
if RECENT_WORKBENCHES == "":
    RECENT_WORKBENCHES = str(DefaultSettings["RecentWorkbenches"])
    Settings.SetStringSetting("RecentWorkbenches", RECENT_WORKBENCHES)
