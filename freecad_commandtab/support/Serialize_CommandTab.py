...
def iconToBase64(icon: QIcon, sz=None, mode=QIcon.Mode.Normal, state=QIcon.State.On):
    """
...
def deserializeIcon(iconPixmaps):
    ico = QIcon()
    for _strW, wPixmaps in iconPixmaps.items():
        for _strH, hPixmaps in wPixmaps.items():
