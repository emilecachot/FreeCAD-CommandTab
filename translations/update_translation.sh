...
update_locale() {
	...
	local -a lupdate_args=(
		"-extensions" "py,ui,inl,cpp,c,cxx,h,hpp"
	)

	mapfile -d '' source_files < <(
		find .. -type f \
			\( -name "*.py" -o -name "*.ui" -o -name "*.inl" -o -name "*.cpp" -o -name "*.c" -o -name "*.cxx" -o -name "*.h" -o -name "*.hpp" \) \
			! -name "CommandTabResources.py" \
			! -path "../dist/*" \
			! -path "../.git/*" \
			-print0
	)
	...
	if [ ${#source_files[@]} -eq 0 ]; then
		echo "No source files found for translation update."
		return 1
	fi
	...
	if [ "$u" == "" ]; then
		"$LUPDATE" "${lupdate_args[@]}" "${source_files[@]}" -ts "${WB}.ts" # locale-agnostic file
	else
		"$LUPDATE" "${lupdate_args[@]}" "${source_files[@]}" \
			-source-language en_US -target-language "${locale//-/_}" \
			-ts "${WB}_${locale}.ts"
	fi
}
